...
function clearDemo() {
  $(".demo").empty();
  $('.demo').html('<label for="pageTitle">页面title<span class="text-danger">[必填]</span>：</label> \n<input type="text" class="form-control" name="pageTitle" id="pageTitle">');
}
...
$(document).ready(function () {
  ...
  var initEvents = function(){
    ...
    $(document).keydown(function(e){
      ...
      var selected,container = $('#elmComponents'),searchContainer;
      if(e.keyCode==13){
        if($.trim(name) == ''){
        ...
        if($('#searchItem').is( ":focus" )){
          ...
          if(searchContainer.length == 0){
            container.append('<div class="search-container"></div>');
          }else{
            searchContainer.html('');
          }

          selected = $('#elmComponents').find('div[name*="' +name + '"]');
          if(selected.length){
            searchContainer.css({
              display: 'block',
              position: 'absolute',
            ...
            searchContainer.append(selected.clone());
            searchContainer.find('.box').draggable({
              connectToSortable: ".column",
              helper: "clone",
              handle: ".drag",
              appendTo: 'body',
            ...
            eval(selected.children('script').html());
          }
        }
      }
    });
  ...
  $.ajax({
    ...
    $('#elmComponents').addClass('clearfix');
